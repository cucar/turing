 ...
 */
function TuringSelectField({ id, label, validators, value, error, options, apiOptions, onChange }) {
	
	let apiResponse = useContext(ApiContext);
	let selectOptions = options || apiOptions(apiResponse);

	return (
		<FormControl style={{ width: '100%' }} error={!!error}>
			<InputLabel htmlFor={id}>{label}</InputLabel>
			<Select value={value.toString()} onChange={onChange} inputProps={{ name: id, id: id }}>
				{selectOptions.map(option => <MenuItem key={option.value.toString()} value={option.value.toString()}>{option.label}</MenuItem>)}
			</Select>
